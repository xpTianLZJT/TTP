...
>
本文要求所有列车仅在起始站由容量、尺寸和技术标准相同的**列车单元**进行编组，允许在给定长度范围 $[\underline{m}_i, \overline{m}_i]$ 内灵活选择编组长度。对于列车 $i \in I$ ，最小长度 $\underline{m}_i$ 为站台长度，最大长度 $\overline{m}_i$ 由可用列车单元数量决定，并用集合 $U_i = \{1, 2, \dots, \overline{m}_i\}$ 表示列车 $i$ 可能使用的所有单元。对于每列超长列车，不同的停站位置会导致列车与站台不同的对齐关系。如图3所示，对于站台长度为6、最大列车长度为10的案例，存在五种可能的对齐关系。为便于表述，本文将这种列车与站台的对齐关系简称为**列车对齐方案**。用集合 $J^s _i$ 为列车 $i$ 在车站 $s$ 的所有候选对齐方案， $j\in J^s _i$ 。根据车站客运组织实践，乘客上下车活动须在站台内进行。为保证乘客安全顺利上下车，仅允许对齐站台的列车单元开启车门，而超出站台的车厢必须保持关闭。如图3所示，对于对齐方案 $j=1$ ，列车单元2至7对齐站台，则要求这些单元的车门开启，以便乘客上下车，其他未与站台对齐的列车单元车门必须关闭，禁止乘客进出。
>
![图3 列车对齐方案](./figure/3.png)
<center>图3 列车对齐方案</center>

>
由于高峰时段车厢内拥挤，乘客在车厢间移动极为困难。因此，本文假设每位乘客只能选择唯一的列车单元完成出行，不允许任何换乘行为。依据乘客上下车要求，乘客所选乘的列车单元必须在起讫站对齐站台，以便车门开启，确保乘客顺利乘降。如图 4 所示，由于列车单元 1 的车门在车站 1 和 4 均可开启，乘客 P2 可在车站 1 乘坐此车厢，并在车站 4 下车。此外，假设单元 1 已达到其容量上限，则无需在车站 2 和 3 对齐站台，以减少无效停靠需求。换言之，在超长列车运营过程中，客流需求加载须满足以下两个条件：(1) 乘客所选择的列车单元必须在其起始站和终到站对齐站台，以确保车门开启；(2) 每个列车单元的在车人数不得超过其载客能力限制。
>
...
> 本文利用超长列车的目的是增加地铁运输能力、减少过饱和需求等待时间。在该问题中，涉及以下三类决策：1）每列列车编组长度；2）所有列车在各车站的对齐方案；3）客流控制方案。对应地，引入以下三种决策变量进行模型构建。

|     符号      |                             描述                             |
| :-----------: | :----------------------------------------------------------: |
|  $x_{i}$          |    列车 $i$ 的编组长度， $x _i \in \{\underline{m} _i,\underline{m} _i +1,· · ·，\overline{m} _i\}$ |
|   $y_{i}^{sj}$    |  如果列车 $i$ 在车站 $s$ 选择对齐方案 $j$ ，则等于 $1$ ；否则等于0                                                                        |
| $b_{iu}^{ss^{'}}$ | 在车站 $s$ 乘坐列车 $i$ 上单元 $u$ 且目的站为车站 $s′$ 站乘客数量  |
...
- **客流控制约束**
>
该约束通过控制乘客上下车活动，合理分配乘客至恰当的列车单元。该过程须满足以下三项要求：其一，乘客所乘列车单元的门必须在起讫站开启，以保障乘客顺利乘降；其二，严格遵守各列车单元的载客容量限制，确保列车单元不超载；其三，在任一时间，要求出发乘客累积量不超过到达乘客累积量，保证时变客流的到站规律。针对上述客流控制要求，构建以下约束：
>
$$
...
$$
>
为清楚起见，将上述模型记为M0。需要说明的是，模型 M0 的解空间随问题规模的增长呈指数级扩展，同时，其内部蕴含大量对称性，使得对于中大规模问题，直接采用优化求解器或标准 Benders 分解算法求解存在较大挑战。详细分析见正文。为此，本文对模型 M0 进行重构，并设计列生成算法进行求解。
## 列生成算法
### 模型重构
首先，本文将列车在所有车站上对齐方案的一种可行组合定义为一**列**。如图5所示，列 $p$ 和 $p'$ 表示列车 $i$ 的两种超长列车运行方案，其中列 $p$ 和 $p'$ 对应编组长度分别为10和8。
>
![图5 基于列的对齐方案示例](./figure/5.png)
<center>图5 基于列的对齐方案示例</center>

在以上表述，该问题核心可描述为：如何为每一列车选择最恰当的对齐方案列。因此，引入基于对齐方案列的决策变量：
>
$z_{ip}$ ：如果对齐方案列 $p$ 被列车 $i$ 使用则等于1，否则等于0。
>
利用新引入的变量，将模型M0重新为模型M1，如下所示：
...
与模型M0相比，模型M1包含指数级变量 $z_{ip}$ 。根据模型特点，采用列生成算法进行求解，其中通过考虑列集合的子集 $\bar{P} _i \subseteq P _i$，以及线性松弛整数变量 $(0 ≤ z _{ip} ≤ 1, b^{ss'} _{iu} ≥ 0)$， 构建限制主问题(RMP)，并利用RMP对偶解，进一步构造定价子问题，以生成潜在列更新RMP。
...
## 分支定价算法
一般而言，列生成算法通常仅能获得原问题线性松弛解，需要进一步结合分支定界搜索整数解。在每个分支节点，利用列生成算法获得其最优松弛解（记为下界），如果下界大于当前最优上界，则对节点进行修剪；否则，将进一步分支。具体分支策略如下所述。
### 分支策略
本文采用双层分支策略，即首先分支每列列车使用的单元数量，然后分支每个站点的对齐方案。给定分支节点，用 $ z _{ip}^* $ 表示非整数解，$ l _i^* = \sum _{p \in P _i} n _{ip} \cdot z _{ip}^* $ 表示列车 $ i $ 的编组长度。首先，如果 $ l _i^* $ 为非整数且 $ l _i^* - |l _i^*| $ 最接近 0.5，则对列车 $ i $ 的单元数量进行分支。其次，如果所有列车的 $ l _i^* $ 都为整数，则需判断是否对列车 $ i $ 在车站 $ s $ 的对齐方案 $ j $ 进行分支。需要指出的是，每次分支都需调整限制主问题和定价子问题，细节请见正文。

### 加速策略
- **潜在列生成**
>
在列生成迭代过程中，若能预先生成一些潜在优质列，将有助于加速算法收敛。结合问题特点，本文提出如图7所示的启发式算法，以生成潜在优质列。
>
![潜在列生成](./figure/7.png)
<center>图7 潜在列生成</center>

- **解空间缩减**
>
利用算法2得到的潜在列车对齐方案，构建了一个欠饱和需求曲线（如图8所示），并以此来缩减解空间，加速算法收敛，具体细节详见正文。
>
...
> - 小规模算例
>
针对一条含有4个车站的地铁线路，共有10列列车从车站1运行到车站4。通过变化最大列车长度 $(\overline{m}_i)$ 设置五个不同算例，计算结果如表3所示。很明显，随着最大单元数的增加，乘客等待时间显著减少。
>
![不同最大列车单元数实例的计算结果](./figure/9.png)

为进一步表明列车长度对乘客等待时间的影响，图10展示了Case 1-4的乘客等待过程。其中，Case 1的剩余乘客数最多，Case 4的剩余乘客数最少，验证了超长列车能够有效减少乘客等待时间。
>
...
> - 真实算例
>
以北京地铁八通线为例，验证所提方法的有效性，线路结构与客流需求如图12所示。为了比较分析，考虑了三种不同场景：XLT-SP表示所有乘客具有同等上车优先权；XLT-DP表示上游车站乘客较下游车站乘客具有更高的上车优先权；RT-FIFO表示所有列车长度均为6节编组，且乘客遵循FIFO规则乘车。利用所提方法，对以上三种场景进行计算，优化结果如图13-15所示。
>
...
>
此外，本文考虑多组不同规模算例，验证以下四种求解方法的计算效率。其中，G-M0方法表示直接利用GUROBI求解模型M0，SBD-M0方法表示采用标准Benders分解求解模型M0，CG-H-M1方法表示结合列生成与启发式求解模型M1，B&P-M1方法为本文所提的分支定价算法。从表6可知，本文所提B&P-M1方法能够有效求解不同规模问题。
>
